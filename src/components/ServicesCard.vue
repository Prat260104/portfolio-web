<template>
  <div :class="$attrs.class">
    <div
      class="heading-2 max-sm:heading-3 flex w-full items-center justify-between gap-5 py-6 text-left font-semibold sm:py-5"
    >
      <div class="flex items-center gap-6 sm:gap-12 md:gap-16">
        <span class="self-center text-nowrap">
          ( {{ `0${number}` }} )
        </span>
      ...
      <div
        v-html="shape"
        class="heading-size-3 hidden shrink-0 animate-[spin_10s_linear_infinite] fill-flax-smoke-400/50 lg:block ml-4"
      />
    </div>
